...
# FunciÃ³n helper para obtener el device apropiado
def get_device():
    """Retorna el device apropiado (DirectML, CUDA o CPU)"""
    if HAS_DIRECTML:
...
class ACD(nn.Module):
    ...
    def __init__(self, args, trg_vocab):
        ...
        # Guardar el device para uso posterior
    ...
    def ddim_sample(self, encoder_output, input_3d, src_mask, trg_mask):
        ...
        # Usar self.device en lugar de 'cuda:0'
        ...
        for time, time_next in time_pairs:
            ...
            # DEBUG: Print values to check for encoding/corruption issues
            print('DEBUG alpha:', alpha)
            print('DEBUG alpha_next:', alpha_next)
            print('DEBUG eta:', eta)
            print('DEBUG type alpha:', type(alpha))
            print('DEBUG type alpha_next:', type(alpha_next))
            print('DEBUG type eta:', type(eta))
    ...
    def prepare_diffusion_concat(self, pose_3d):
        ...
        # Usar self.device en lugar de 'cuda'
        t = torch.randint(0, self.num_timesteps, (1,), device=self.device).long()
        noise = torch.randn(pose_3d.shape[0], 150, device=self.device)
