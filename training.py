...
try:
    ...
    HAS_DIRECTML = False
    print("Warning: torch_directml not installed. Using CPU or CUDA if available.")
...
class TrainManager:
 ...
 # CPU / GPU
        ...
        if self.use_cuda:
            if HAS_DIRECTML:
                # Usar DirectML para AMD
                self.device = torch_directml.device()
                self.model.to(self.device)
                self.loss.to(self.device)
                self.logger.info("Using DirectML device for AMD GPU")
    ...
    def save_skels(self, output_joints, display, model_dir, type, file_paths=None):
        ...
        csvIn = pd.read_csv(
            "./Configs/%s_phoenix2014t.csv" % type, sep="|", encoding="utf-8"
                )
        pickle_list = []
        ...
        for i in display:
            ...
            sign = seq.detach().clone() if isinstance(seq, torch.Tensor) else torch.tensor(seq, dtype=torch.float32)
